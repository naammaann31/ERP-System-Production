...
const aggregationLogic = `  const teamReports = reports.filter(r => teamMembers.some(tm => tm.uid === r.user_id));
...
if (!content.includes('const aggregatedReports =')) {
    // Insert before `return (` that is followed by `<motion.div`
    // using a more flexible regex
    content = content.replace(/return\s*\(\s*<motion\.div/, aggregationLogic + '\n  return (\n    <motion.div');
    fs.writeFileSync(file, content);
    console.log("Successfully inserted aggregationLogic!");
} else {
    console.log("aggregationLogic is already there.");
}
