...
import GenerateReportModal from "@/components/dashboard/operations/GenerateReportModal";
import { classifyOperationsImportRow } from "@/lib/operationsExcelImport";
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                let dupCount = 0;
                let invalidCount = 0;
                const internalNameFallback = profile?.fullName || profile?.email || "";

                for (const row of rawData as any[]) {
                    const decision = classifyOperationsImportRow(row, data, internalNameFallback);
                    ...
                    const { error } = await supabase
                        .from(collectionName)
                        .insert(salesUiToRow(decision.payload!, profile?.uid || null));
                    if (error) throw error;
                    newCount++;
                }
...
<table className="w-full text-sm text-left whitespace-nowrap relative">
            ...
            <GenerateReportModal
                ...
                onSubmit={handleGenerateMasterReport}
